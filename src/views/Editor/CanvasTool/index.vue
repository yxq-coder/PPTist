<template>
  <div class="canvas-tool">
    <div class="left-handler">
      ...
      <Tooltip :mouseLeaveDelay="0" :mouseEnterDelay="0.5" title="保存">
        <IconSaveOne class="handler-item" @click="save()" />
      </Tooltip>
      <Tooltip :mouseLeaveDelay="0" :mouseEnterDelay="0.5" title="预览">
        <IconPreviewOpen size="16" class="handler-item" @click="enterScreening()" />
      </Tooltip>
    </div>
    ...
    <div class="add-element-handler">
      ...
      <Popover trigger="click" v-model:visible="imageInputVisible">
        <template #content>
          <FileInput 
            @close="imageInputVisible = false"
            @insertFile="src => { createImageElement(src); imageInputVisible = false }"
          />
        </template>
       ...
       <Popover trigger="click" v-model:visible="pdfInputVisible">
        <template #content>
          <FileInput 
            ...
            @insertFile="src => { createPdfElement(src); pdfInputVisible = false }"
          />
        </template>
        <Tooltip :mouseLeaveDelay="0" :mouseEnterDelay="0.5" title="插入pdf">
          <IconFilePdf class="handler-item" />
        </Tooltip>
      </Popover>
    ...
    <div class="right-handler">
      ...
      <Tooltip :mouseLeaveDelay="0" :mouseEnterDelay="0.5" title="关闭">
        <IconPower size="15" style="margin-left:20px" class="handler-item" @click="exit()" />
      </Tooltip>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const save = () => {
      if (window['$cef']) {
        (window['$cef'] as any).saveSlides(JSON.stringify(slides))
      }
      else {
