<template>
  <div class="slide-design-panel">
    ...
    <div class="row">
      ...
      <Select 
        style="flex: 10;" 
        :value="background.imageSize || 'contain'" 
    ...
    <div class="background-image-wrapper" v-if="background.type === 'image'">
      ...
      <Popover trigger="click" v-model:visible="imageInputVisible">
        <template #content>
          <ImageInput :src="background.image"
            @close="imageInputVisible = false"
            @insertFile="src => { uploadBackgroundImage(src); imageInputVisible = false }"
          />
        </template>
        <div class="background-image">
          <div class="content" :style="{ backgroundImage: `url(${background.image})` }">
    ...
    <div class="row">
      ...
      >
        ...
        <SelectOptGroup label="在线字体" v-if="false">
...
import ImageInput from '@/views/Editor/CanvasTool/FileInput.vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const updateBackgroundType = (type: 'solid' | 'image' | 'gradient') => {
      if (type === 'solid') {
        ...
        const newBackground: SlideBackground = {
          ...
          imageSize: background.value.imageSize || 'contain',
        }
        slidesStore.updateSlide({ background: newBackground })
      }
      else {
    ...
    const uploadBackgroundImage = (src: string) => {
